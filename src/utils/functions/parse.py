...
regex_split_operators = (
    to_regex(operators3) + to_regex(operators2) + to_regex(operators1)
)
    ...
    for line in gadget:
        ...
        for fun_name in user_fun:
            if (
                ...
                hex_line = re.sub(
                    r"\b(" + fun_name + r")\b(?=\s*\()", fun_symbols[fun_name], hex_line
                )

        for var_name in user_var:
            # next line is the nuanced difference between fun_name and var_name
            if (
                len({var_name[0]}.difference(keywords)) != 0
                and len({var_name[0]}.difference(main_args)) != 0
            ):
